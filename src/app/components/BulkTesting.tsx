    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        <div style={{
          ...
          background: hasResult
            ? (passed ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.12)")
        ...
        <div style={{
          flex: 1, height: 1,
          background: hasResult
            ? (passed ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.15)")
            : "rgba(99,102,241,0.1)",
        }} />
        {hasResult && agentDone && (
      ...
      <div style={{
        ...
        {hasResult && agentDone && result?.issues?.length > 0 && (
          <div style={{
            padding: "8px 16px", borderTop: "1px solid rgba(239,68,68,0.12)",
            background: "rgba(239,68,68,0.04)",
            animation: "fadeInScore 0.5s ease 0.3s both",
          }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{
        ...
        border: "1px solid rgba(255,255,255,0.75)", borderRadius: 20,
        padding: "20px 24px",
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 5 }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 800, color: "#111827", letterSpacing: "-0.3px" }}>Bulk Testing</span>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
        {criteria.map(c => {
          ...
          return (
            <div key={c.id} onClick={() => !isUniversal && toggle(c.id)} style={{
              ...
              <div style={{ fontSize: 10, color: "rgba(60,40,120,0.45)" }}>
                {isUniversal ? "🔒 Always tested" : c.selected ? "✓ Will be tested" : "Skipped"}
              </div>
            </div>
          );
        })}
        ...
        {addingCustom ? (
          ...
          <div onClick={() => setAddingCustom(true)} style={{
            background: "rgba(255,255,255,0.25)", border: "1.5px dashed rgba(99,102,241,0.25)", borderRadius: 16, padding: 16,
            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, cursor: "pointer", minHeight: 120, transition: "all 0.2s",
          }}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "fadeIn 0.6s ease both" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {[
              ...
              <div key={i} style={{
                background: "rgba(255,255,255,0.6)", borderRadius: 12, padding: "10px 14px",
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 22px", borderBottom: "1px solid rgba(255,255,255,0.5)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" as const }}>
                    ...
                    <span style={{
                      ...
                      {passed ? <CheckCircle size={8} /> : <XCircle size={8} />}
                      {passed ? "Pass" : "Fail"}
                    </span>
                    <span style={{ fontSize: 9, color: "rgba(99,102,241,0.5)", padding: "2px 7px", borderRadius: 20, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.12)" }}>
                      Content {Math.round(result.content_score ?? 0)}
                    </span>
                    <span style={{ fontSize: 9, color: "rgba(8,145,178,0.7)", padding: "2px 7px", borderRadius: 20, background: "rgba(8,145,178,0.05)", border: "1px solid rgba(8,145,178,0.12)" }}>
                      Behavior {Math.round(result.behavior_score ?? 0)}
                    </span>
                  </div>
                  {result.summary && (
                    <div style={{ fontSize: 11, color: "rgba(60,40,120,0.6)", lineHeight: 1.5 }}>{result.summary}</div>
                  )}
                  {result.issues?.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 4, marginTop: 5 }}>
                      {result.issues.map((issue, j) => (
                        <span key={j} style={{ fontSize: 9, padding: "2px 7px", borderRadius: 20, background: "rgba(239,68,68,0.06)", color: "#EF4444", border: "1px solid rgba(239,68,68,0.15)", fontWeight: 600 }}>{issue}</span>
                      ))}
                    </div>
                  )}
                </div>

                <div style={{ fontSize: 9, color: "rgba(99,102,241,0.35)", flexShrink: 0, marginTop: 2 }}>
                  {isExpanded ? "▲" : "▼"}
                </div>
              </div>
  ...
  async function handleRun() {
    ...
    try {
      const { data: runData, error } = await supabase
        .from("test_runs")
        .insert({ workspace_id: userId, agent_id: prompt.id, agent_type: "booking", status: "running", attempt_number: 1 })
        .select("id").single();
      ...
      await fetch(BULK_WEBHOOK, {
        ...
        body: JSON.stringify({ user_id: userId, agent_id: prompt.id, test_run_id: runData.id, criteria: selected.map(c => ({ id: c.id, text: c.text, type: c.type })) }),
      });
