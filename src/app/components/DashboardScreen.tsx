...
const LAYERS = [
  ...
  { key: "magnetSent"   as const, label: "Lead Magnet Sent", color: "#A5B4FC" },
...
const LEADS = [
  { init:"AK", name:"Ahmed Karimi",  step:"Booking",     status:"Booked",      sBg:"#EEF2FF", sColor:"#4F46E5", date:"Apr 25", aBg:"#EEF2FF", aColor:"#4F46E5" },
  { init:"SM", name:"Sara Moreira",  step:"Lead Magnet", status:"Magnet Sent", sBg:"#DCFCE7", sColor:"#16A34A", date:"Apr 24", aBg:"#DCFCE7", aColor:"#16A34A" },
  { init:"JD", name:"John Davidson", step:"Booking",     status:"Booked",      sBg:"#EEF2FF", sColor:"#4F46E5", date:"Apr 24", aBg:"#EEF2FF", aColor:"#4F46E5" },
  { init:"MS", name:"Maria Santos",  step:"Script 1",    status:"New",         sBg:"#DBEAFE", sColor:"#2563EB", date:"Apr 23", aBg:"#DBEAFE", aColor:"#2563EB" },
  { init:"CL", name:"Chris Lee",     step:"Booking",     status:"No Show",     sBg:"#FEE2E2", sColor:"#DC2626", date:"Apr 22", aBg:"#FEE2E2", aColor:"#DC2626" },
];
...
function Avatar({ init, bg="#EEF2FF", color="#4F46E5", size=32 }: {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:"50%", background:bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:size*0.34, fontWeight:600, color, flexShrink:0, fontFamily:"'Plus Jakarta Sans',sans-serif", letterSpacing:"-0.3px" }}>
      {init}
    </div>
  );
}
      ...
      <div style={{ display:"flex", alignItems:"flex-end", gap:8, height:BAR_H + 28 }}>
        {data.map((entry, i) => {
          ...
          return (
            ...
            >
              <div style={{ width:"100%", height:scale(entry.leadsHandled), position:"relative", cursor:"pointer", borderRadius:"8px 8px 0 0", overflow:"hidden" }}>
                {LAYERS.map((layer) => (
                  <div key={layer.key} style={{ position:"absolute", bottom:0, left:0, right:0, height:scale(entry[layer.key]), background:layer.color, borderRadius:"8px 8px 0 0", transition:"height .3s" }} />
                ))}
              </div>
              <span style={{ fontSize:10, color:"#9CA3AF", fontWeight:400 }}>{labels[i]}</span>
            </div>
          );
        })}
      </div>
        ...
        <div style={{ position:"absolute", left:tooltip.x, top:tooltip.y-8, transform:"translate(-50%,-100%)", background:"#111827", borderRadius:12, padding:"12px 16px", pointerEvents:"none", zIndex:99, minWidth:190, boxShadow:"0 12px 32px rgba(0,0,0,.25)" }}>
          ...
          {LAYERS.map(layer => (
            <div key={layer.key} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
              ...
              <span style={{ fontSize:13, color:"#fff", fontWeight:600 }}>{data[tooltip.idx][layer.key]}</span>
            </div>
          ))}
      ...
      <div style={{ display:"flex", gap:20, marginTop:16, flexWrap:"wrap" }}>
        {LAYERS.map(l => (
          <div key={l.key} style={{ display:"flex", alignItems:"center", gap:6 }}>
            ...
            <span style={{ fontSize:11, color:"#9CA3AF", fontWeight:400 }}>{l.label}</span>
          </div>
        ))}
      </div>
    </div>
...
export function DashboardScreen() {
  ...
  const stats = [
    { label:"Leads Handled",    value:"142", change:"+18%", up:true,  color:"#4F46E5", Icon: Users         },
    { label:"Lead Magnet Sent", value:"98",  change:"+12%", up:true,  color:"#16A34A", Icon: Mail          },
    { label:"Booked Calls",     value:"34",  change:"+8%",  up:true,  color:"#2563EB", Icon: CalendarCheck },
    { label:"Show Up Rate",     value:"76%", change:"+5%",  up:true,  color:"#D97706", Icon: CheckSquare   },
  ];
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .ds-stat:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(0,0,0,.1) !important; }
        .ds-flow { transition: transform .2s, box-shadow .2s; cursor: pointer; }
        .ds-flow:hover { transform: translateY(-3px); box-shadow: 0 10px 28px rgba(0,0,0,.09) !important; }
        .ds-row:hover { background: #F9FAFB !important; }
        .ds-per { transition: all .15s; }
        .ds-per:hover { background: #E5E7EB !important; }
        .ds-icon-btn { transition: all .15s; }
        .ds-icon-btn:hover { background: #F3F4F6 !important; }
        .ds-view-all:hover { background: #F9FAFB !important; }
      `}</style>

      <div style={{ padding:"28px 32px", width:"100%", boxSizing:"border-box", fontFamily:"'Plus Jakarta Sans',sans-serif", background:"#F3F4F8", minHeight:"100vh" }}>

        {/* Top Bar */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28 }}>
          <div>
            <h1 style={{ fontSize:22, fontWeight:700, color:"#111827", letterSpacing:"-0.5px", marginBottom:3 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div className="ds-icon-btn" style={{ width:38, height:38, borderRadius:10, background:"#fff", border:"1px solid #E5E7EB", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"background .15s" }}>
              <Search size={15} strokeWidth={1.8} color="#6B7280" />
            </div>
            <div className="ds-icon-btn" style={{ width:38, height:38, borderRadius:10, background:"#fff", border:"1px solid #E5E7EB", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", position:"relative", transition:"background .15s" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 10px 6px 6px", background:"#fff", border:"1px solid #E5E7EB", borderRadius:12, cursor:"pointer" }}>
              ...
              <div>
                <div style={{ fontSize:12, fontWeight:600, color:"#111827", lineHeight:1.2 }}>LeadFlow</div>
                <div style={{ fontSize:10, color:"#9CA3AF", lineHeight:1.2 }}>Admin</div>
              </div>
              <ChevronDown size={13} color="#9CA3AF" strokeWidth={1.8} />
            </div>
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
          {stats.map((s, i) => (
            <div key={i} className="ds-stat" style={{ background:"#FFFFFF", borderRadius:16, padding:"20px 20px 16px", boxShadow:"0 1px 4px rgba(0,0,0,.05)", position:"relative", overflow:"hidden" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
                <div style={{ width:36, height:36, borderRadius:10, background: i===0?"#EEF2FF":i===1?"#DCFCE7":i===2?"#DBEAFE":"#FEF3C7", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <s.Icon size={16} strokeWidth={1.8} color={s.color} />
                </div>
                <span style={{ fontSize:10, fontWeight:600, color: s.up?"#16A34A":"#DC2626", background: s.up?"#DCFCE7":"#FEE2E2", padding:"3px 9px", borderRadius:20, display:"flex", alignItems:"center", gap:3 }}>
                  <TrendingUp size={9} strokeWidth={2} /> {s.change}
                </span>
              </div>
              <div style={{ fontSize:32, fontWeight:800, color:"#111827", letterSpacing:"-1.5px", lineHeight:1, marginBottom:5 }}>{s.value}</div>
              <div style={{ fontSize:12, color:"#6B7280", fontWeight:500, marginBottom:14 }}>{s.label}</div>
              <button style={{ fontSize:10, fontWeight:600, color:"#4F46E5", border:"none", background:"none", padding:0, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif", display:"flex", alignItems:"center", gap:3 }}>
                View details <ArrowUpRight size={10} strokeWidth={2} />
              </button>
              {/* bottom accent line */}
              <div style={{ position:"absolute", bottom:0, left:0, right:0, height:3, background:`linear-gradient(90deg,${s.color},${s.color}44)` }} />
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:16, marginBottom:16 }}>
          ...
          <div style={{ background:"#FFFFFF", borderRadius:16, padding:"22px 24px", boxShadow:"0 1px 4px rgba(0,0,0,.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
              <div>
                <div style={{ fontSize:15, fontWeight:700, color:"#111827", marginBottom:3, letterSpacing:"-0.3px" }}>Performance Overview</div>
                <div style={{ fontSize:11, color:"#9CA3AF", fontWeight:400 }}>Leads through your flow</div>
              </div>
              <div style={{ display:"flex", gap:4, background:"#F3F4F6", padding:4, borderRadius:10 }}>
                {(["weekly","monthly","yearly"] as const).map(p => (
                  <button key={p} className="ds-per" onClick={() => setPeriod(p)} style={{ padding:"5px 14px", borderRadius:7, border:"none", fontSize:11, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:600, transition:"all .15s", background:period===p ? "#FFFFFF" : "transparent", color:period===p ? "#111827" : "#9CA3AF", boxShadow:period===p ? "0 1px 4px rgba(0,0,0,.1)" : "none" }}>
          ...
          <div style={{ background:"#FFFFFF", borderRadius:16, padding:"22px 20px", boxShadow:"0 1px 4px rgba(0,0,0,.05)", display:"flex", flexDirection:"column" }}>
            ...
            <div style={{ fontSize:11, color:"#9CA3AF", fontWeight:400, marginBottom:18 }}>All latest events</div>
            <div style={{ flex:1 }}>
              {ACTIVITIES.map((a, i) => (
                <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"9px 0", borderBottom:i < ACTIVITIES.length-1 ? "1px solid #F3F4F6" : "none" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:10, color:"#9CA3AF", fontWeight:400, marginTop:2 }}>{a.time}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Leads Table */}
        <div style={{ background:"#FFFFFF", borderRadius:16, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,.05)", marginBottom:16 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"18px 24px", borderBottom:"1px solid #F3F4F6" }}>
            <div>
              <div style={{ fontSize:15, fontWeight:700, color:"#111827", letterSpacing:"-0.2px" }}>Recent Leads</div>
              <div style={{ fontSize:11, color:"#9CA3AF", fontWeight:400, marginTop:2 }}>Latest leads in your pipeline</div>
            </div>
            <button style={{ padding:"8px 16px", borderRadius:10, border:"none", background:"#111827", color:"#fff", fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif", display:"flex", alignItems:"center", gap:5 }}>
              <ArrowUpRight size={12} strokeWidth={2} /> View All
            </button>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1.5fr 1fr 1fr", padding:"10px 24px", background:"#F9FAFB", borderBottom:"1px solid #F3F4F6" }}>
            {["Lead Name","Flow Step","Status","Date"].map(col => (
              <div key={col} style={{ fontSize:10, color:"#9CA3AF", textTransform:"uppercase", letterSpacing:"1px", fontWeight:600 }}>{col}</div>
            ))}
          </div>
          {LEADS.map((lead, i) => (
            <div key={i} className="ds-row" style={{ display:"grid", gridTemplateColumns:"2fr 1.5fr 1fr 1fr", padding:"13px 24px", borderBottom:i < LEADS.length-1 ? "1px solid #F9FAFB" : "none", cursor:"pointer", transition:"background .1s" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, fontSize:13, fontWeight:600, color:"#111827" }}>
                <Avatar init={lead.init} bg={lead.aBg} color={lead.aColor} size={28} />
                {lead.name}
              </div>
              <div style={{ display:"flex", alignItems:"center", fontSize:12, color:"#6B7280", fontWeight:400 }}>{lead.step}</div>
              <div style={{ display:"flex", alignItems:"center" }}>
                <span style={{ fontSize:10, fontWeight:600, color:lead.sColor, background:lead.sBg, padding:"4px 10px", borderRadius:20 }}>{lead.status}</span>
              </div>
              <div style={{ display:"flex", alignItems:"center", fontSize:11, color:"#9CA3AF", fontWeight:400 }}>{lead.date}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background:"#FFFFFF", borderRadius:16, padding:"22px 24px", boxShadow:"0 1px 4px rgba(0,0,0,.05)" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
            <div>
              <div style={{ fontSize:15, fontWeight:700, color:"#111827", letterSpacing:"-0.2px", marginBottom:3 }}>Lead Flow Sequence</div>
              <div style={{ fontSize:11, color:"#9CA3AF", fontWeight:400 }}>Your current automation pipeline</div>
            </div>
            <button style={{ padding:"8px 16px", borderRadius:10, border:"none", background:"#111827", color:"#fff", fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
              Edit Flow
            </button>
          </div>
          ...
          <div style={{ display:"flex", alignItems:"stretch" }}>
            {FLOW_STEPS.map((step, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", flex:1 }}>
                <div className="ds-flow" style={{ flex:1, borderRadius:14, border:"1px solid #E5E7EB", background:"#FFFFFF", position:"relative", overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,.04)" }}>
                  {/* Top accent */}
                  <div style={{ height:3, background:step.color, width:"100%" }} />

                  {/* Face image for Booking, otherwise icon */}
                  {step.img ? (
                    <div style={{ height:90, overflow:"hidden", background:"#EEF2FF" }}>
                  ...
                  <div style={{ padding: step.img ? "12px 14px 14px" : "10px 14px 14px" }}>
                    <div style={{ fontSize:13, fontWeight:700, color:"#111827", marginBottom:2, letterSpacing:"-0.2px" }}>{step.label}</div>
                    <div style={{ fontSize:10, color:"#9CA3AF", fontWeight:400, marginBottom:10 }}>{step.sub}</div>
          ...
          {/* Conversion Bar */}
          <div style={{ marginTop:18, display:"flex", alignItems:"center", gap:16, background:"#F9FAFB", borderRadius:12, padding:"13px 18px", border:"1px solid #F3F4F6" }}>
            ...
            <div style={{ flex:1, height:6, background:"#E5E7EB", borderRadius:10, overflow:"hidden" }}>
              <div style={{ height:"100%", width:"24%", background:"linear-gradient(90deg,#4F46E5,#7C3AED)", borderRadius:10 }} />
            </div>
            <div style={{ fontSize:13, fontWeight:700, color:"#111827", flexShrink:0 }}>24% (34/142)</div>
          </div>
        </div>
