...
const FLOW_STEPS = [
  { label: "Trigger", sub: "WhatsApp", color: "#8B5CF6", bg: "#F5F3FF", border: "#DDD6FE", icon: "⚡", count: "142" },
  { label: "Script 1", sub: "Welcome", color: "#3B82F6", bg: "#EFF6FF", border: "#BFDBFE", icon: "💬", count: "138" },
  { label: "Lead Magnet", sub: "Send PDF", color: "#10B981", bg: "#ECFDF5", border: "#A7F3D0", icon: "🎁", count: "98" },
  { label: "Script 2", sub: "Follow-up", color: "#F59E0B", bg: "#FFFBEB", border: "#FDE68A", icon: "📞", count: "61" },
  { label: "Booking", sub: "Schedule", color: "#EF4444", bg: "#FFF1F2", border: "#FECDD3", icon: "📅", count: "34" },
];
    ...
    <div style={{ padding: "36px 40px", width: "100%", boxSizing: "border-box" }}>
        ...
        {stats.map((s, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, position: "relative" }}>
              ...
              <button style={{ padding: "5px 13px", borderRadius: 20, background: "rgba(255,255,255,0.88)", border: "none", fontSize: 11, fontWeight: 600, color: "#374151", cursor: "pointer" }}>
                View details
              </button>
            </div>
            ...
            <div style={{ position: "absolute", bottom: 18, right: 18, display: "flex", alignItems: "center", gap: 3, background: "rgba(255,255,255,0.25)", padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, color: "#fff" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20, marginBottom: 20 }}>
            ...
            {data.map((val, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                  <div style={{
                    width: "100%", height: h,
                    background: isMax ? "linear-gradient(180deg,#818CF8,#6366F1)" : "linear-gradient(180deg,#E0E7FF,#C7D2FE)",
        ...
        <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E5E7EB", padding: "26px 24px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)", display: "flex", flexDirection: "column" }}>
          ...
          <button style={{ width: "100%", marginTop: 16, padding: "10px", borderRadius: 10, border: "1px solid #E5E7EB", background: "#FAFBFE", fontSize: 11, fontWeight: 600, color: "#6366F1", cursor: "pointer" }}>
            View All Activity
          </button>
        </div>
      </div>
      ...
      {/* Recent Leads Table */}
      <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E5E7EB", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 28px", borderBottom: "1px solid #E5E7EB" }}>
          ...
          <button style={{ padding: "8px 18px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 4px 12px rgba(99,102,241,0.3)" }}>
        ...
        {LEADS.map((lead, i) => {
          ...
          return (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg,#EEF2FF,#E0E7FF)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#6366F1", flexShrink: 0 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E5E7EB", padding: "26px 28px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#0F1117", marginBottom: 3 }}>Lead Flow Sequence</div>
            <div style={{ fontSize: 12, color: "#9CA3AF" }}>Your current automation pipeline</div>
          </div>
          <button style={{ padding: "8px 18px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", boxShadow: "0 4px 12px rgba(99,102,241,0.3)" }}>
            Edit Flow
          </button>
        </div>
          ...
          {FLOW_STEPS.map((step, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", flex: 1 }}>
              <div style={{
                ...
                onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = "translateY(0)"}
              >
                <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg,${step.color},${step.color}88)`, borderRadius: "16px 16px 0 0" }} />
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                  <div style={{ fontSize: 22 }}>{step.icon}</div>
                  <div style={{ width: 7, height: 7, borderRadius: "50%", background: step.color, boxShadow: `0 0 8px ${step.color}` }} />
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#0F1117", marginBottom: 2 }}>{step.label}</div>
                <div style={{ fontSize: 11, color: "#6B7280", marginBottom: 12 }}>{step.sub}</div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ fontSize: 24, fontWeight: 800, color: step.color, letterSpacing: "-1px" }}>{step.count}</div>
                  <div style={{ fontSize: 9, fontWeight: 700, color: step.color, background: `${step.color}18`, padding: "3px 8px", borderRadius: 20 }}>leads</div>
                </div>
              </div>
              {i < FLOW_STEPS.length - 1 && (
                <div style={{ display: "flex", alignItems: "center", padding: "0 10px", flexShrink: 0 }}>
                  <div style={{ width: 32, height: 2, background: "linear-gradient(90deg,#E5E7EB,#D1D5DB)" }} />
                  <div style={{ width: 0, height: 0, borderTop: "5px solid transparent", borderBottom: "5px solid transparent", borderLeft: "7px solid #D1D5DB" }} />
                </div>
              )}
            </div>
          ))}
        ...
        {/* Conversion bar */}
        <div style={{ marginTop: 24, padding: "16px 20px", background: "#F8F9FC", borderRadius: 12, display: "flex", alignItems: "center", gap: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", flexShrink: 0 }}>Conversion rate</div>
          <div style={{ flex: 1, height: 8, background: "#E5E7EB", borderRadius: 10, overflow: "hidden" }}>
            <div style={{ height: "100%", width: "24%", background: "linear-gradient(90deg,#6366F1,#8B5CF6)", borderRadius: 10, boxShadow: "0 2px 8px rgba(99,102,241,0.4)" }} />
          </div>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#6366F1", flexShrink: 0 }}>24% (34/142)</div>
        </div>
      </div>
